...
export type GenesisTier = 'genesis_80' | 'genesis_119' | 'genesis_176';

function pricePointFromTier(tier: GenesisTier): '80' | '119' | '176' {
  if (tier === 'genesis_80') return '80';
  if (tier === 'genesis_119') return '119';
  return '176';
}
...
export async function reserveGenesisSlot(admin: SupabaseClient, tier: GenesisTier) {
  ...
  const defaultCap =
    tier === 'genesis_80'
      ? GENESIS_SLOT_LIMITS.genesis80
      : tier === 'genesis_119'
        ? GENESIS_SLOT_LIMITS.genesis119
        : GENESIS_SLOT_LIMITS.genesis176;
  return {
    ok: true as const,
    sold: payload.slots_used ?? 0,
    cap: payload.slots_total ?? defaultCap,
  };
}
...
export async function getGenesisInventory(admin: SupabaseClient) {
  ...
  if (error || !data) {
    return {
      ...
      genesis176: { sold: 0, cap: GENESIS_SLOT_LIMITS.genesis176, remaining: GENESIS_SLOT_LIMITS.genesis176 },
    };
  }
  const row80 = data.find((r: { price_point: string }) => r.price_point === '80');
  const row119 = data.find((r: { price_point: string }) => r.price_point === '119');
  const row176 = data.find((r: { price_point: string }) => r.price_point === '176');
  ...
  const sold176 = Number(row176?.slots_used ?? 0);
  const cap176 = Number(row176?.slots_total ?? GENESIS_SLOT_LIMITS.genesis176);
  return {
    ...
    genesis176: { sold: sold176, cap: cap176, remaining: Math.max(0, cap176 - sold176) },
  };
}
