...
export const PRELAUNCH_PROMISE_REGISTRY: PromiseSection[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'If standard prices change later, grandfathering follows the locked checkout amounts and Paddle subscription items configured for that checkout.',
    ],
  },
  {
    ...
    bullets: [
      ...
      'Mechanism: automated Paddle Billing refund adjustments where `PADDLE_API_KEY` is configured; otherwise support is notified via `email_outbox` for manual completion.',
  ...
  },
  {
    ...
    bullets: [
      ...
      'Annual: full year charged upfront at the discounted annual totals shown; renewal reverts to standard pricing after the first annual term as configured in Paddle.',
      'LTD: one-time charge today; no recurring billing for the lifetime entitlement.',
      'LTD + Sage bundle: the live app may open sequential Paddle checkouts until a single bundled Paddle price is configured — both paths are logged to `paddle_transactions` for reconciliation.',
  ...
  },
  {
    ...
    bullets: [
      ...
      'Email 1.5: Recovery — `email_1_5_recovery_setup` at ≥6h and ≥24h for orphan Paddle rows without a linked profile (EC-04).',
      ...
      'Support templates: `email_support_alert_unclaimed_payment`, `email_support_paddle_refund_failed`, `email_support_paddle_cancel_failed`, `email_ec03_webhook_delay_support`.',
    ],
  },
];
