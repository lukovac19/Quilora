-- =============================================================================
-- Quilora — final consolidated setup (migrations 0001–0012, excluding 0010)
-- Safe when ONLY public.profiles exists (with data). Never drops profiles.
-- Order: extensions → legacy drops → helper function + grant → profile ALTERs
--   → tables → indexes → business functions (each + immediate grant) → triggers
--   → RLS → policies → seeds → storage → table grants
-- =============================================================================

-- (a) Extensions
create extension if not exists pgcrypto;

-- (b) Drop legacy objects from 0001 / prior runs (IF EXISTS, CASCADE)
...
-- (c) Helper function — no table references in body; grant immediately after
...
-- ---------------------------------------------------------------------------
-- public.profiles — extend only (no DROP/CREATE; no new CHECK constraints)
-- ---------------------------------------------------------------------------
alter table public.profiles drop constraint if exists profiles_tier_check;
alter table public.profiles drop constraint if exists profiles_billing_plan_check;
alter table public.profiles drop constraint if exists profiles_subscription_status_check;
alter table public.profiles drop constraint if exists profiles_genesis_slot_price_point_check;
...
alter table public.profiles add column if not exists prelaunch_holding boolean not null default true;
alter table public.profiles add column if not exists first_prelaunch_purchase_at timestamptz;
alter table public.profiles add column if not exists prelaunch_cancelled_at timestamptz;
alter table public.profiles add column if not exists checkout_email_verified_at timestamptz;
alter table public.profiles add column if not exists genesis_slot_price_point text;
alter table public.profiles add column if not exists genesis_lifetime_discount boolean not null default false;
...
-- genesis_slots: no FK to other app tables — create early so RPCs / %rowtype / seeds never hit 42P01
...
-- 0011 upgrade path (must run before CREATE of dodo_webhook_dedup / provider_transactions)
do $$
begin
  if exists (
...
-- (d) Main tables — merged from 0002–0009, 0011–0012 (Dodo-era names; no paddle_* creates)
...
-- genesis_slots: allow 80 / 119 / 176 (add named CHECK only if missing on this table)
do $$
begin
  if not exists (
    select 1 from information_schema.tables
...
-- (f)(g) Business logic functions — each followed immediately by GRANT
...
begin
  ...
  update public.sandboxes sandbox
  set read_only = true
  from ranked ranked_row
  where sandbox.id = ranked_row.id and ranked_row.rn > 5;
end;
$$;
...
set search_path = public
as $$
<<enforce_bookworm_sandbox_limit_fn>>
declare
  active_sandbox_count integer;
  user_tier text;
begin
  if tg_op = 'INSERT' and new.is_deleted = false then
    select prof.tier into enforce_bookworm_sandbox_limit_fn.user_tier
    from public.profiles prof where prof.id = new.user_id;
    if enforce_bookworm_sandbox_limit_fn.user_tier = 'bookworm' then
      select count(*) into active_sandbox_count
      from public.sandboxes sandbox
      where sandbox.user_id = new.user_id and sandbox.is_deleted = false;
      if active_sandbox_count >= 5 then
        raise exception 'SANDBOX_LIMIT_BOOKWORM' using errcode = 'P0001';
      end if;
    end if;
  end if;
  return new;
end enforce_bookworm_sandbox_limit_fn;
$$;

grant execute on function public.enforce_bookworm_sandbox_limit() to service_role;
...
begin
  if auth.role() <> 'service_role' then
    if current_auth_uid is null or current_auth_uid <> p_user_id then
  ...
  if p_idempotency_key is not null then
    if exists (
      select 1 from public.credit_events credit_evt
      where credit_evt.user_id = p_user_id and credit_evt.idempotency_key = p_idempotency_key
    ) then
      select prof.credit_balance into balance_before from public.profiles prof where prof.id = p_user_id;
      return jsonb_build_object('ok', true, 'duplicate', true, 'credit_balance', balance_before);
    end if;
  end if;

  select prof.credit_balance into balance_before from public.profiles prof where prof.id = p_user_id for update;
  if balance_before is null then
    raise exception 'PROFILE_NOT_FOUND';
  end if;

  balance_after := balance_before + p_credits_delta;
  ...
  spend_amount := abs(p_credits_delta);

  update public.profiles set credit_balance = balance_after, updated_at = now() where id = p_user_id;
  ...
  return jsonb_build_object('ok', true, 'duplicate', false, 'credit_balance', balance_after);
end;
$$;
...
declare
  ...
  previous_last_streak_date date;
  next_streak_count integer;
  profile_streak_goal integer;
begin
  if auth.role() <> 'service_role' and (current_auth_uid is null or current_auth_uid <> p_user_id) then
    raise exception 'NOT_ALLOWED';
  end if;

  select prof.last_streak_date, prof.streak_count, prof.streak_goal
  into previous_last_streak_date, next_streak_count, profile_streak_goal
  from public.profiles prof where prof.id = p_user_id for update;

  if previous_last_streak_date is null then
    next_streak_count := 1;
  elsif previous_last_streak_date <> today_utc then
    if previous_last_streak_date = today_utc - 1 then
      next_streak_count := next_streak_count + 1;
    else
      next_streak_count := 1;
    end if;
  end if;
  ...
  set last_streak_date = today_utc,
      streak_count = next_streak_count,
      updated_at = now()
  where id = p_user_id;

  return jsonb_build_object('ok', true, 'streak_count', next_streak_count, 'streak_goal', profile_streak_goal);
end;
$$;
...
as $$
declare
  genesis_slot_row public.genesis_slots%rowtype;
begin
  if auth.role() <> 'service_role' then
    raise exception 'NOT_ALLOWED';
  end if;

  select genesis_tbl.* into genesis_slot_row
  from public.genesis_slots genesis_tbl
  where genesis_tbl.price_point = p_price_point for update;
  if not found then
    return jsonb_build_object('ok', false, 'reason', 'NOT_FOUND');
  end if;
  if genesis_slot_row.slots_used >= genesis_slot_row.slots_total then
  ...
  return jsonb_build_object(
    'ok', true,
    'slots_used', genesis_slot_row.slots_used + 1,
    'slots_total', genesis_slot_row.slots_total
  );
end;
$$;
...
begin
  ...
  if genesis_slot_row.slots_used <= 0 then
...
grant execute on function public.profiles_enforce_locked_columns() to service_role;
...
-- (i)(j) Row level security + policies
...
create policy sandbox_content_all on public.sandbox_content for all using (
  exists (select 1 from public.sandboxes sandbox where sandbox.id = sandbox_id and sandbox.user_id = auth.uid())
) with check (
  exists (select 1 from public.sandboxes sandbox where sandbox.id = sandbox_id and sandbox.user_id = auth.uid())
);
